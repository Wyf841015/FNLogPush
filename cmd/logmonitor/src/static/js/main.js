...
async function exportConfig() {
    try {
        ...
        let filename = 'fnlogpush_config.json';
...
// 导入配置（选择文件后）
async function importConfig(fileInput) {
    ...
    try {
        ...
        if (!data.version || !data.config) {
            showNotification('无效的配置文件格式', 'danger');
            fileInput.value = '';
            return;
        }
        
        // 显示导入确认模态框
        showConfigImportConfirm(data, fileInput);
        
    } catch (e) {
        showNotification('读取配置文件失败: ' + e.message, 'danger');
        fileInput.value = '';
    }
}

// 显示配置导入确认模态框
function showConfigImportConfirm(data, fileInput) {
    const modal = new bootstrap.Modal(document.getElementById('configImportModal'));
    const infoList = document.getElementById('import-config-info');
    const confirmCheck = document.getElementById('confirm-import-check');
    const confirmBtn = document.getElementById('confirm-import-btn');
    
    // 显示配置信息
    let html = '<li>配置版本: <strong>' + esc(data.version || '未知') + '</strong></li>';
    if (data.export_time) {
        html += '<li>导出时间: <strong>' + esc(data.export_time) + '</strong></li>';
    }
    if (data.config) {
        html += '<li>config.json: <span class="text-success">包含 ' + Object.keys(data.config).length + ' 个配置项</span></li>';
    }
    if (data.events) {
        html += '<li>events.json: <span class="text-success">包含 ' + data.events.length + ' 个事件定义</span></li>';
    }
    if (data.manual) {
        html += '<li><span class="badge bg-info">手动备份</span></li>';
    }
    ...
    // 保存数据引用
    window._importConfigData = data;
    window._importFileInput = fileInput;
...
async function confirmImportConfig() {
    const data = window._importConfigData;
    const fileInput = window._importFileInput;
    
    if (!data) {
        showNotification('导入数据丢失', 'danger');
        return;
    }
    ...
    try {
        ...
        if (result.success) {
            showNotification(result.message || '配置导入成功', 'success');
            ...
            showNotification(result.error || '导入失败', 'danger');
        }
    } catch (e) {
        showNotification('导入失败: ' + e.message, 'danger');
    } finally {
        window._importConfigData = null;
        window._importFileInput = null;
        if (fileInput) fileInput.value = '';
    }
}

// 显示配置备份历史模态框
function showConfigBackupModal() {
    const modal = new bootstrap.Modal(document.getElementById('configBackupModal'));
    modal.show();
    loadBackupHistory();
}
...
async function loadBackupHistory() {
    ...
    tbody.innerHTML = '<tr><td colspan="3" class="text-center"><i class="fas fa-spinner fa-spin"></i> 加载中...</td></tr>';
    ...
    try {
        ...
        if (result.success && result.backups.length > 0) {
            ...
            result.backups.forEach(function(b) {
                ...
                html += '<tr>';
                html += '<td>' + esc(date.toLocaleString('zh-CN')) + '</td>';
                html += '<td>' + size + '</td>';
                html += '<td>';
                html += '<button class="btn btn-sm btn-outline-primary me-1" onclick="restoreBackup(\'' + esc(b.name) + '\')" title="恢复此备份"><i class="fas fa-undo"></i></button>';
                html += '</td>';
                html += '</tr>';
            });
            tbody.innerHTML = html;
        } else {
            tbody.innerHTML = '<tr><td colspan="3" class="text-center text-muted">暂无备份记录</td></tr>';
        }
    } catch (e) {
        tbody.innerHTML = '<tr><td colspan="3" class="text-center text-danger">加载失败: ' + esc(e.message) + '</td></tr>';
    }
}
...
async function createManualBackup() {
    try {
        const response = await fetch('/api/config/backup/create', {method: 'POST'});
        const result = await response.json();
        ...
        if (result.success) {
            ...
            showNotification(result.error || '创建备份失败', 'danger');
        }
    } catch (e) {
        showNotification('创建备份失败: ' + e.message, 'danger');
    }
}
...
async function restoreBackup(filename) {
    ...
    try {
        const response = await fetch('/api/config/backup/restore/' + encodeURIComponent(filename), {
            method: 'POST'
        });
        const result = await response.json();
        
        if (result.success) {
            showNotification(result.message || '配置已恢复', 'success');
            bootstrap.Modal.getInstance(document.getElementById('configBackupModal')).hide();
            ...
            showNotification(result.error || '恢复失败', 'danger');
        }
    } catch (e) {
        showNotification('恢复失败: ' + e.message, 'danger');
    }
}
...
window.showConfigBackupModal = showConfigBackupModal;
